...
/*! \brief function for GraphExecutor */
...
static int TVM_RT_WASM_GraphExecutorLoad(const char *graph_json, TVMModuleHandle module_handle, const DLDevice *devices,
 ...
 */
int TVM_RT_WASM_GraphExecutorGetNumOfNodes(const TVM_RT_WASM_GraphExecutor g) {
 ...
 * \return 0 if successful
 */
int TVM_RT_WASM_GraphExecutorGetNodeName(const TVM_RT_WASM_GraphExecutor g, uint32_t nid, const char **name) {
    CHECK_GraphExecutor(g);
    if (unlikely(nid > g->num_nodes)) {
        SET_ERROR_RETURN(-1, "invalid argument: nid, expect it in range [0,%d), but given %d", g->num_nodes, nid);
    }
 ...
 */
int TVM_RT_WASM_GraphExecutorGetInputIndex(const TVM_RT_WASM_GraphExecutor g, const char *name) {
    CHECK_GraphExecutor(g);
    if (unlikely(name == NULL)) {
        SET_ERROR_RETURN(-1, "invalid argument: name, must not be NULL");
    }
 ...
 */
int TVM_RT_WASM_GraphExecutorGetOutputIndex(const TVM_RT_WASM_GraphExecutor g, const char *name) {
    CHECK_GraphExecutor(g);
    if (unlikely(name == NULL)) {
        SET_ERROR_RETURN(-1, "invalid argument: name, must not be NULL");
    }
 ...
 */
int TVM_RT_WASM_GraphExecutorGetNumInputs(const TVM_RT_WASM_GraphExecutor g) {
    CHECK_GraphExecutor(g);
 ...
 */
int TVM_RT_WASM_GraphExecutorGetNumOutputs(const TVM_RT_WASM_GraphExecutor g) {
    CHECK_GraphExecutor(g);
...
int TVM_RT_WASM_GraphExecutorSetInput(TVM_RT_WASM_GraphExecutor g, uint32_t index, const DLTensor *data_in) {
    ...
    if (unlikely(index > g->num_inputs_nodes)) {
        SET_ERROR_RETURN(-1, "invalid argument: index, expect it in range [0,%d), but given %d", g->num_inputs_nodes,
                         index);
    }
...
int TVM_RT_WASM_GraphExecutorGetOutput(TVM_RT_WASM_GraphExecutor g, uint32_t index, DLTensor *data_out) {
    CHECK_GraphExecutor(g);
    if (unlikely(index > g->num_outputs)) {
        SET_ERROR_RETURN(-1, "invalid argument: out_puts, expect it in range [0,%d), but given %d", g->num_outputs,
                         index);
    }
 ...
 * \return 0 if successful
 */
int TVM_RT_WASM_GraphExecutorDestory(TVM_RT_WASM_GraphExecutor *g) {
    if (unlikely(g == NULL)) {
        SET_ERROR_RETURN(-1, "invalid param: the GraphExecutorManager pointer cannot be NULL");
    }
    TVM_RT_WASM_GraphExecutor graph = *g;
    CHECK_GraphExecutor(graph);
    *g = NULL; // set handle to NULL
    ...
    // free extension data
    if (graph->Destory) {
        graph->Destory(graph->extension_data);
    }
